...
const float K1           = 4; // Screen distance
const float K2           = 6; // Object distance
const float widthScale   = 1.0;
const vec3  L            = {0, -2, 1.2}; // Light position
const int   numSurfaces  = 8;
    ...
    for (int i = 0; i < numSurfaces; ++i) {
        for (float v = -1; v < 1; v += vDelta) {
            for (float u = -1; u < 1; u += uDelta) {
                ...
                const vec3 coordsObjRot = {cosPhi * coordsObj[0] - sinPhi * coordsObj[1], sinPhi * coordsObj[0] + cosPhi * coordsObj[1], coordsObj[2]};

                const vec3 coordsObjRotTra = {coordsObjRot[0] + K2, coordsObjRot[1], coordsObjRot[2]};
                ...
                const vec3  NR         = {cosPhi * N[0] - sinPhi * N[1], sinPhi * N[0] + cosPhi * N[1], N[2]};
...
bool dither(float l, int x, int y) {
    static const std::map<float, std::function<bool(int, int)>> ditherMap = {{0.0f, [](int x, int y) { return false; }}, {0.11f, [](int x, int y) { return x % 3 == 0 && y % 3 == 0; }}, {0.25f, [](int x, int y) { return x % 2 == 0 && y % 2 == 0; }}, {0.33f, [](int x, int y) { return ( x + y ) % 3 == 0; }}, {0.5f, [](int x, int y) { return ( x + y ) % 2 == 0; }}, {0.66f, [](int x, int y) { return ( x + y ) % 3 != 0; }}, {0.75f, [](int x, int y) { return x % 2 == 0 || y % 2 == 0; }}, {0.88f, [](int x, int y) { return x % 3 != 0 || y % 3 != 0; }}, {1.0f, [](int x, int y) { return true; }}};
...
void printOledDataToConsole(
    std::function<vec3(float, float, int)> uv,
    std::function<vec3(vec3, int)> normal
) {
    float phi = 0;
    ...
    for (int i = 0; i < numFrames; ++i) {
        phi += phiDelta;
        std::cout << "{";
        std::cout << convertOLEDStringToHex(
            convertFrameToOLED(
                generateFrame(phi, uv, normal)
            )
        );
        std::cout << "},\n";
    }
...
vec3 defaultNormal(vec3 coords, int i) {
    ...
    return normalize({coords[0], coords[1], static_cast<float>(magnitude(coords[0], coords[1]) * (widthScale * pi / 2.0f * sin(pi * coords[2])))});
}
...
vec3 torusNormal(vec3 coords, int i) {
    ...
    return normalize({coords[0], static_cast<float>(y - r0 * cos(theta)), static_cast<float>(z - sin(theta))});
}
...
int main() {
    ...
    printOledDataToConsole(defaultUV, defaultNormal);
}
